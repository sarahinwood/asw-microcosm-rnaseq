...
dds_location$group <- factor(paste(dds$Weevil_Location))
dds_location$parasitism <- factor(paste(dds$Parasitism_status))

##add group to design
design(dds_location) <- ~group
##run deseq2 and generate results
dds_location <- DESeq(dds_location)
##save dds_group
saveRDS(dds_location, file = "output/deseq2/asw/WT_location/asw_dds_location.rds")

resultsNames(dds_location)

##Make table of results for exposed vs control heads
res_group <- results(dds_location, contrast = c("group", "Ruakura", "Dunedin"), lfcThreshold = 1, alpha = 0.1)
##Order based of padj
...
##write tables
fwrite(ordered_res_group_table, "output/deseq2/asw/WT_location/full_res.csv")
fwrite(ordered_sig_res_group_table, "output/deseq2/asw/WT_location/sig_degs.csv", col.names = TRUE, row.names = FALSE)
...
fwrite(sig_degs_annots, "output/deseq2/asw/WT_location/sig_degs_annots.csv")
...
vd <- venn.diagram(x = list("LRT location"=lrt_location$rn, "WT location"=wt_location$rn), filename=NULL, alpha=0.5, cex = 1, cat.cex=1, lwd=1,)
grid.newpage()
grid.draw(vd)
