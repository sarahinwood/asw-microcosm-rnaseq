...
plotCounts(dds_location_parasitism, "ASW_TRINITY_DN657_c0_g1", intgroup = c("Parasitism", "Location"), main="")

dds_location_parasitism$loc_para <- factor(paste(dds_location_parasitism$Weevil_Location, dds_location_parasitism$Parasitism_status, sep="_"))
##plot expression pattern for gene
plot_gene <- plotCounts(dds_location_parasitism, "ASW_TRINITY_DN2983_c1_g1", 
                        intgroup = c("loc_para"), returnData = TRUE)
ggplot(plot_gene,
       aes(x = loc_para, y = count)) + 
...
fwrite(annot_sig_fgsea, "output/deseq2/asw/WT_location-parasitism_int/fgsea_sig_annot.csv")
