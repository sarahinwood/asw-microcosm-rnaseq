...
sig_gene_names <- fread("output/deseq2/asw/WT_location-attacked_int/sig_gene_names.csv", header=FALSE)

##vst log transform data - absolute counts now changed so cannot compare between genes (only between samples for 1 gene)
vst <- varianceStabilizingTransformation(dds_location_attacked, blind = FALSE)
...
long_coldata <- data.table(as.data.frame(colData(dds_location_attacked)))
...
mean_vst <- merged_exp_coldata[,.(vst_mean = gm_mean(vst)),by=.(rn, loc_att)]
##make long table a wide table instead
mean_vst_wide <- dcast(mean_vst, rn~loc_att)
