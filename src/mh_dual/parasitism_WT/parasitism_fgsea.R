...
trinotate_report <- fread("data/asw-mh-combined-transcriptome/output/mh_edited_transcript_ids/trinotate_annotation_report.txt", na.strings=".")
##fairly sure we always used Pfam GO annots for this
gene_ids <- trinotate_report[!is.na(gene_ontology_Pfam), unique(`#gene_id`)]
res_group <- fread("output/deseq2/mh_dual/parasitism_WT/res_group.csv")
...
fwrite(annot_sig_fgsea, "output/deseq2/mh_dual/parasitism_WT/sig_GO_enrichment.csv")
