...
##finds genes not clearly DE jsut based off location - stick with WT
##differences in gene expression based on ASW source location
##controlling for other factors

dds <- readRDS("output/deseq2/asw/asw_dds.rds")
dds_location <- copy(dds)

dds_location$Location <- factor(dds_location$Weevil_Location)
dds_location$Parasitism <- factor(dds_location$Parasitism_status)
dds_location$Sample_cleaned <- factor(dds_location$Cleaned)

##Look for genes DE between locations, controlling for parasitism status
design(dds_location) <- ~Sample_cleaned+Parasitism+Location
dds_location <- DESeq(dds_location, test='LRT', reduced=~Sample_cleaned+Parasitism)
saveRDS(dds_location, "output/deseq2/asw/LRT_location/dds_location.rds")

dds_location <- readRDS("output/deseq2/asw/LRT_location/dds_location.rds")

##extract results
dds_res <- results(dds_location, alpha=0.1)

##results for all genes - for FGSEA analysis
full_res_dt <- data.table(data.frame(dds_res), keep.rownames=TRUE)
fwrite(full_res_dt, "output/deseq2/asw/LRT_location/LRT_location_para_full_res.csv")

##make list of sig genes
sig_degs <- subset(dds_res, padj<0.05)
sig_degs_names <- row.names(sig_degs)
fwrite(data.table(sig_degs_names), "output/deseq2/asw/LRT_location/sig_degs_names.csv")
...
fwrite(ordered_sig_degs_table, "output/deseq2/asw/LRT_location/sig_degs.csv")
##merge with annots
asw_trinotate <- fread("data/asw_longest_isoform_annots.csv")
ordered_sig_degs_annots <- merge(ordered_sig_degs_table, asw_trinotate, by.x="fixed_ids", by.y="#gene_id", all.x=TRUE, all.y=FALSE)
fwrite(ordered_sig_degs_annots, "output/deseq2/asw/LRT_location/sig_degs_annots.csv")

##plot counts for genes of interest, sub in name
plotCounts(dds_location, "ASW_TRINITY_DN12172_c3_g1", intgroup = c("Parasitism", "Location"))
...
fwrite(annot_sig_fgsea, "output/deseq2/asw/LRT_location/sig_annot_fgsea_pfam.csv")
